...
import { getPopularShows, searchShows, getShowDetails } from './lib/tmdb.js';

export default async function handler(req, res) {
  await handleRequest(req, res, async () => {
    if (!process.env.TMDB_API_KEY) {
      return errorResponse(res, 503, 'TV service not available', 'TMDB_API_KEY is not configured');
    }
    ...
    try {
      if (type === 'popular') {
        const result = await getPopularShows(pageNum);
      ...
      if (type === 'search') {
        ...
        const result = await searchShows(query, pageNum);
        return successResponse(res, result);
      }
